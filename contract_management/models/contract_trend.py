...
class ContractDashboardTrend(models.Model):
    ...
    sequence = fields.Integer(string='Secuencia', default=10, help='Orden de evaluación (menor a mayor).')
    
    metric_type = fields.Selection([
        ('active_contracts', 'Contratos Activos'),
        ('total_revenue', 'Valor Total (Ingresos)'),
        ('expiring_soon', 'Próximos a Vencer')
    ], string='Tarjeta a evaluar', required=True)
    
    comparison_period = fields.Selection([
        ('7', 'Últimos 7 días'),
        ('15', 'Últimos 15 días'),
        ('30', 'Últimos 30 días')
    ], string='Período de Comparación', required=True, default='30')
    ...
    operator = fields.Selection([
        ...
        ('>', 'Mayor a (>)'),
        ('<', 'Menor a (<)')
    ], string='Condición', required=True, default='>=')
    
    percentage = fields.Float(string='Porcentaje Umbral (%)', required=True, default=0.0)
    ...
    ], string='Color del Porcentaje', required=True, default='text-success')
    
    details = fields.Text(string='Detalles (Tooltip)', translate=True, help='Texto que aparecerá al pasar el mouse sobre el porcentaje.')
