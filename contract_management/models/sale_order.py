...
class SaleOrder(models.Model):
    ...
    contract_count = fields.Integer(compute='_compute_contract_ids', string='Contract Count')
    ...
    def action_view_contracts(self):
        ...
        if len(contracts) == 1:
            action['views'] = [(self.env.ref('contract.contract_contract_form_view').id, 'form')]
            action['res_id'] = contracts.id.id if hasattr(contracts.id, 'id') else contracts.id
        else:
