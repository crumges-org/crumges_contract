...
class ContractTemplate(models.Model):
    ...
    tag_ids = fields.Many2many("contract.tag", string="Etiquetas")
    ...
    def _compute_contract_counts(self):
        for template in self:
            ...
            template.count_in_progress = len(
                contracts.filtered(lambda c: c.state == "in_progress")
            )
            template.count_done = len(contracts.filtered(lambda c: c.state == "done"))
            template.count_cancelled = len(
                contracts.filtered(lambda c: c.state == "cancelled")
            )
            template.count_paused = len(
                contracts.filtered(lambda c: c.state == "paused")
            )
    ...
    def _get_action(self, state=None):
        ...
        if self.contract_type == "purchase":
            action = self.env["ir.actions.act_window"]._for_xml_id(
                "contract_management.action_supplier_invoice_contract"
            )
            context = {
                "default_contract_type": "purchase",
                "default_generation_type": "invoice",
                "default_purchase_generation_type": "invoice",
                "default_contract_template_id": self.id,
                "default_auto_post_invoice": self.auto_post_invoice,
            }
        elif (
            self.contract_type == "sale"
            and getattr(self, "generation_type", "invoice") == "sale"
        ):
            action = self.env["ir.actions.act_window"]._for_xml_id(
                "contract_management.action_customer_sale_contract"
            )
            context = {
                "default_contract_type": "sale",
                "default_generation_type": "sale",
                "default_sale_generation_type": "sale",
                "default_contract_template_id": self.id,
                "default_sale_autoconfirm": self.sale_autoconfirm,
            }
        else:
            action = self.env["ir.actions.act_window"]._for_xml_id(
                "contract_management.action_customer_invoice_contract"
            )
            context = {
                "default_contract_type": "sale",
                "default_generation_type": "invoice",
                "default_sale_generation_type": "invoice",
                "default_contract_template_id": self.id,
                "default_auto_post_invoice": self.auto_post_invoice,
            }
