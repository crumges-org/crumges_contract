...
class ContractManagementCustomerPortal(CustomerPortal):
    def _get_invoices_domain(self, *args, **kwargs):
        domain = (
            super()._get_invoices_domain(*args, **kwargs)
            if hasattr(super(), "_get_invoices_domain")
            else []
        )
    ...
    def _prepare_orders_domain(self, partner, *args, **kwargs):
        domain = (
            super()._prepare_orders_domain(partner, *args, **kwargs)
            if hasattr(super(), "_prepare_orders_domain")
            else []
        )
        contract_id = request.params.get("contract_id")
        if contract_id:
            try:
                domain = expression.AND(
                    [
                        ...
                        [
                            (
                                "order_line.contract_line_id.contract_id",
                                "=",
                                int(contract_id),
                            )
                        ],
    ...
    def _prepare_quotations_domain(self, partner, *args, **kwargs):
        domain = (
            super()._prepare_quotations_domain(partner, *args, **kwargs)
            if hasattr(super(), "_prepare_quotations_domain")
            else []
        )
        contract_id = request.params.get("contract_id")
        if contract_id:
